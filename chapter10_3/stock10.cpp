...
Stock2::~Stock2() { std::cout << "Bye, " << company << "!\n"; }

void Stock2::buy(long num, double price) {
  if (num < 0) {
    std::cout << "Number of shares purchased can't be negative. Transaction is "
                 "aborted.\n";
  } else {
    shares += num;
    share_val = price;
...
void Stock2::sell(long num, double price) {
  using std::cout;
  if (num < 0) {
    cout << "Number of shares purchased can't be negative. Transaction is "
            "aborted.\n";
  } else if (num > shares) {
    cout << "You can't sell more than you have! Transaction is abroted.\n";
  } else {
    shares -= num;
    share_val = price;
    set_tot();
